...
def get_gen(gen: list) -> list:
    ...
    и change_table, ген приводится к виду списка, в котором находятся два
...
def table_for_individual(individual: list) -> list:
    ...
    В последовательности seq, если в гене присутствуют комплексные вершины,
    ...
    Алгоритм составления таблицы:
        ...
        Для этого необходимо:
            ...
            TL;DR: Нужно взять среднее арифметические мер близости одной
            вершины относительно второй (порядок не важен,
            т.к. таблица симметрична).
